...
const ExcelDownload = ({ list }) => {
  ...
  const Changed_Excel_Data = async () => {
    if (list.Bom_Lists.length) {
      const Nothing_Info_Part_Lists = list.Bom_Lists.filter(
        (item) => item.ERP_PART.startsWith("R") && item.Purchase_price === 0,
      );
      const Has_Info_Part_Lists = list.Bom_Lists.filter(
        (item) => item.ERP_PART.startsWith("R") && item.Purchase_price > 0,
      );

      const All_Part_Info_Lists =
        Nothing_Info_Part_Lists.concat(Has_Info_Part_Lists);
